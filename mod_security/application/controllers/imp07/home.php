...
class Home extends CI_Controller {
    ...
    function index() {
        $data['url_post'] = site_url('imp07/home/uploaddata');
        $this->load->view('imp07/form', $data);
    }
    ...
    function import_excel($ext = '.xls', $path_file = '/tmp/data.xlsx') {
        ...
        foreach ($rowIterator as $row) {
            ...
            $rowloc = $this->inventaris->getr02_byname($array_data[$rowIndex]['B']);
            $idloc = $rowloc->IDLoc;
            
            
            $record = array(
                'IDInventaris' => $array_data[$rowIndex]['A'],
                'IDLocation' => $idloc,
                'Note' => $array_data[$rowIndex]['C']
            );

            $this->inventaris->checktrx01($array_data[$rowIndex]['A'], $record);
        }
        return $msg;
    }
