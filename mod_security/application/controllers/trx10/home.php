...
class Home extends CI_Controller{
    ...
    function main(){
        $data['waiting'] = '('.$this->rpt->countreport(0)->Nilai.')';       
        $data['solved'] = '('.$this->rpt->countreport(1)->Nilai.')';
        $data['suspension'] = '('.$this->rpt->countreport(2)->Nilai.')';
        $data['unsolved'] = '('.$this->rpt->countreport(3)->Nilai.')';
        $data['progress'] = '('.$this->rpt->countreport(4)->Nilai.')';        
        $this->load->view("trx10/main",$data);
    }
...
//        $status = $this->input->post("status");
    ...
    function set_report(){
        ...
        if ($action == "solved") {
            ...
            $status = "5";
            $rec['RejectedBy'] = $this->User;
            $rec['RejectedDate'] = $this->Datetime;
            $rec['RejectedIP'] = $this->Ip;
            $rec['RejectReason'] = $snote;
        }
