...
class Home extends CI_Controller {
    ...
    public function __construct() {
        parent ::__construct();
        $this->load->model('Fieldpayroll_model', 'slipthr');
    ...
    function index() {
        $data['url_post'] = site_url('imp11/home/uploaddata');
        $this->load->view('imp11/form', $data);
    }
    ...
    function import_excel($ext = '.xls', $path_file = '/tmp/data.xlsx') {
        ...
        foreach ($rowIterator as $row) {
            ...
            $nip = substr($array_data[$rowIndex]['A'], 1, 10);
            $posting = date('Y-m-d',  strtotime($array_data[$rowIndex]['B']));
                       
            $record = array(
                'IDEmployee' => $nip,
                'PostingDate' => $posting,             
                'SumDailySalaryPayment' => strtoupper($array_data[$rowIndex]['C']),             
            );

            $this->slipthr->checkdataslip($nip,$posting, $record);
        }
        return $msg;
    }
