...
class Home extends CI_Controller {
    ...
    function index() {
        $data['url_post'] = site_url('imp10/home/uploaddata');
        $this->load->view('imp10/form', $data);
    }
    ...
    function import_excel($ext = '.xls', $path_file = '/tmp/data.xlsx') {
        ...
        foreach ($rowIterator as $row) {
            ...
            $record = array(
                ...
                'EmployeeStatus' => strtoupper($array_data[$rowIndex]['B']),             
            );

            $this->employee->updatedata_d($array_data[$rowIndex]['A'], $record);
            $this->employee->updatedata_job($array_data[$rowIndex]['A'], $record);
        }
        return $msg;
    }
