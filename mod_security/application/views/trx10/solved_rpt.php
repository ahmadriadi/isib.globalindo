...
function buildtable(){
    var dataajax    = "<?php echo site_url()?>/trx10/home/get_data/1";
//    alert(dataajax);
    var table = $('#table_solvedrpt').dataTable({
        "bJQueryUI": false,
        "bSortClasses": false,
        "aaSorting": [[2, "desc" ]],
        "bAutoWidth": false,
	    ...
	    {"mData": "Location", "sTitle": "ID Location", "sClass": "left"},  	
          ...
          //  {"mData": "SolutionDate", "sTitle": "SolutionDate", "sClass": "left"},
            {
                ...
                "mRender": function(a,b,c){
                    ...
                    return ba+c.RName+icon+bb+c.HODC;
                }
                    ...
                    bin = bin+'<table class="table-borderless" width="200px">';
                        bin = bin+'<tr>';
                            bin = bin+'<td rowspan="3" width="50%" style="padding: 0">';
                                bin = bin+'<a disabled="disabled" idrpt="'+c.ID+'" action="solved" onclick="prob_action(this)" class="widget-stats btn-success" style="margin: 0; height: 100%; border-radius: 5px;" > ';
                            ...
                            bin = bin+'<td width="50%" style="padding: 0; padding-left: 5px;">';
                                bin = bin+'<span disabled="disabled" idrpt="'+c.ID+'" action="inprogress" onclick="prob_action(this)" class="tbldalam btn btn-small btn-primary glyphicons cogwheels"><i></i>InProgress</span>';
                            bin = bin+'</td>';
                        bin = bin+'</tr>';
                        bin = bin+'<tr>';
                            bin = bin+'<td style="padding: 0; padding-left: 5px;">';
                                bin = bin+'<span disabled="disabled" idrpt="'+c.ID+'" action="suspended" onclick="prob_action(this)" class="tbldalam btn btn-small btn-warning glyphicons clock"><i></i>Suspended</span>';
                            bin = bin+'</td>';
                        bin = bin+'</tr>';
                        bin = bin+'<tr>';
                            bin = bin+'<td style="padding: 0; padding-left: 5px;">';
                                bin = bin+'<span disabled="disabled" idrpt="'+c.ID+'" action="unsolved" onclick="prob_action(this)" class="tbldalam btn btn-small btn-danger glyphicons remove"><i></i>Unsolved</span>';
                            bin = bin+'</td>';
                        bin = bin+'</tr>';
                        bin = bin+'<tr>';
                            bin = bin+'<td style="padding: 0; padding-top: 5px;" colspan="2">';
                                bin = bin+'<span disabled="disabled" idrpt="'+c.ID+'" action="needconf" onclick="prob_action(this)" class="tbldalamakhir btn btn-small btn-facebook glyphicons share"><i></i>Needs HoD Confirmation</span>';
                            bin = bin+'</td>';
                        bin = bin+'</tr>';
                    bin = bin+'</table>';
                    var btn = "";
...
//            alert(d.toSource());
        ...
        "fnDrawCallback": function(oSettings) {
            ...
            $("#table_solvedrpt>tbody>tr").attr("sub","close");
            $("#table_solvedrpt>tbody>tr").addClass("selectable");
            $("#table_solvedrpt>tbody>tr.selectable").click(function (){
...
<div class="row-fluid">
    <div class="span12">
        <table width="100%" id="table_solvedrpt" class="table-bordered table-condensed table-primary"></table>
    </div>
</div>
