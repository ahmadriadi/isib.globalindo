...
//                    else if (c.HODC == '1'){ icon = "<a class='glyphicons standard btn-small primary ok_2'><i></i>"; ba = "<b>"; bb = "</b></a>";}
	    ...
	    {"mData": "CDate", "sTitle": "Time Since Submission Date", "sClass": "left",
             "mRender": function(a, b, c) {
                 var dater = a.split(" ");
                 var tanggal = dater[0];
                 var jam = dater[1];
                 
                 return showrange(tanggal,jam);   
 	...
 	
            {
                ...
                "mRender": function(a,b,c) {
                    var bin = "";
                    bin = bin+'<div class="btn-group"> <table class="table-borderless " width="200px">';
                        bin = bin+'<tr >';
                            bin = bin+'<td  rowspan="4" width="50%" style="padding: 0">';
                        ...
                        bin = bin+'<tr >';
                            bin = bin+'<td  style="padding: 0; padding-left: 5px;">';
                                bin = bin+'<span idrpt="'+c.ID+'" action="rejected" onclick="prob_action(this)" class="tbldalam btn btn-small btn-inverse glyphicons remove"><i></i>Reject</span>';
                            bin = bin+'</td>';
                        bin = bin+'</tr>';                        
                        bin = bin+'<tr >';
                            bin = bin+'<td  style="padding: 0; padding-top: 5px;" colspan="2">';
                                bin = bin+'<span idrpt="'+c.ID+'" action="needconf" onclick="prob_action(this)" class="tbldalamakhir btn btn-small btn-facebook glyphicons share"><i></i>HoD Confirmation</span>';
                            bin = bin+'</td>';
                        bin = bin+'</tr>';
                    bin = bin+'</table>';
                    var btn = "";
                    ...
                    <?php if (in_array("ITMGR", $upar)){?>
                        ...
                        btn = btn+"class='btn btn-small btn-inverse' data-toggle='popover' data-title=''>Set PIC</span> </div>";                        
                    <?php } ?>
//                    return c.PIC;
                    return btn;
                }
...
function showrange(tanggal,jam){
	var user_date = new Date (tanggal);
	var today_date = new Date();
	var diff_date =  (user_date - today_date);

	var num_years = diff_date/31536000000; //years
	var num_months = (diff_date % 31536000000)/2628000000; //month
	var num_days = ((diff_date % 31536000000) % 2628000000)/86400000; //days


	var now = tanggal+' '+jam;
	var startDate = new Date(now.replace(/-/g,"/"));
	var endDate = new Date(today_date);
	var diffrange = (startDate - endDate); 
	var Hour    = Math.floor(Math.abs((diffrange % 86400000) / 3600000)); // hours
	var Minutes = Math.floor(Math.abs(((diffrange % 86400000) % 3600000) / 60000)); // minutes
        
        var wyear   = Math.floor(Math.abs(num_years));
        var wmonth  = Math.floor(Math.abs(num_months));
        var wday   = (Math.round(Math.abs(num_days)));
