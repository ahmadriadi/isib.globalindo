  ...
  function index() {
	...
	$query = $this->employee->get_rjob()->result();
		...
		$data['accessbutton'] ='true';
                $result = $this->leavepermit->leavepermit_hrd($fromdate, $untildate,'AL');
                $viewon = 'trx02a/homeajax';
            } else if ($role == '1') {
		    ...
		    $data['accessbutton'] ='false';	
                    $result = $this->leavepermit->leavepermit($fromdate, $untildate, $iduser,'AL');
                      $viewon = 'trx02a/homeajax';
   ...
   function autocomplete_employee() {
    ...
    function addpost() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $indate = ($f05==' hh:mm' or trim($f05)==':')?null:$f05;  
            $f06 = $this->anti_xss($this->input->post('f06'));
            $hour = (strtotime($indate) - strtotime($f04)) / 3600;   
            
            if($f05A !==null and $f05B==null or $f05B=='hhmm'){
                $f05 = $f05A.'00:00';
                $hour = (strtotime($f05) - strtotime($f04)) / 3600;   
                $sumhour = ($indate==null)?null:abs($hour);
            }else{
                $sumhour = ($indate==null)?null:abs($hour);
            }
    ...
    function edit($id) {
        ...
        $data['default']['f05A'] = ($row->InDate==null or $row->InDate=='01-01-1970')?'':$indate;
        $data['default']['f05B'] = ($row->InDate==null or $row->InDate=='01-01-1970 00:00:00')?'':$timein;
    ...
    function editpost() {
     ...
     function delete($id) {
        $row = $this->leavepermit->get_by_id($id);
        $nip = $row->IDEmployee;
        $presence = date('Y-m-d',  strtotime($row->OutDate));
        ...
        $this->leavepermit->lastpresenceby_necessity($nip,$presence,$recordlastpresence);
    ...
    function excel($g) {
        ...
        if ($checkdata1 == 'exist' and $checkdata2 == 'exist') {
            ...
            if ($role == '2') {
                $result = $this->leavepermit->leavepermit_hrd($f, $u,$g);
            } else if ($role == '1') {
                $parameter = $rowparam->ParamValue;
                if ($parameter == $iduser) {
                    $result = $this->leavepermit->leavepermit_hrd($f, $u,$g);
                } else {
                    $result = $this->leavepermit->leavepermit($f, $u, $iduser,$g);
                }
            } else if ($role == '0' and $position == 'DIRECTOR' or $position == 'DIREKTUR' or $position == 'ASS. MANAGER' or $position == 'KOMISARIS' or $position == 'PROJECT TEAM LEADER' or $position == 'SUPERVISOR') {
