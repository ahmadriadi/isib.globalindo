...
class Home extends CI_Controller {
  ...
  function anti_xss($source)
	...
	$idmenu                    = "179";
        ...
        $this->load->view('ref04/home', $data);
    }
    ...
    function addnew() {
        $data['default']['f01'][0]['value'] = 'overtime';
        $data['default']['f01'][0]['display'] = 'Overtime';      
        $data['default']['f02'] = '25'; //From Date
        $data['default']['f03'] = '25'; //Until Date
        $data['default']['f04'] = ''; //User Exclude
        $data['default']['f05'] = 'hhmm';  //Limit time User
    ...
    function addpost() {
        $this->form_validation->set_rules('f02', 'Value 1', 'required');
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $rcheck = $this->paramlock->checkdata($f01);
            if ($rcheck == 'exist') {
                $alert = 'insert failed, paramlock  ' . $f01 . ' already exist';
                $status = 'false';
            } else {
                $this->paramlock->insert($record);
    ...
    function edit($id) {
        ...
        $row = $this->paramlock->get_by_id($id);
        $type = $row->IDParam;
        if ($type == 'overtime') {
            $data['default']['f01'][0]['value'] = 'overtime';
            $data['default']['f01'][0]['display'] = 'Overtime';          
        } else {
            $data['default']['f01'][0]['value'] = null;
            $data['default']['f01'][0]['display'] = '-';      
        }
        $data['default']['f02'] = $row->Val1; //
        $data['default']['f03'] = $row->Val2; //
        $data['default']['f04'] = $row->Val3; //
        $data['default']['f05'] = $row->Val4; //
    ...
    function editpost() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $this->paramlock->update($id, $record);
            $this->logs->insert($recordlog);
    ...
    function excel() {
        ...
        // rename the sheet
        $objSheet->setTitle('paramlock report');
        ...
        // write header
        $objSheet->getCell('A1')->setValue('Parameter');
        $objSheet->getCell('B1')->setValue('Value 1');
        $objSheet->getCell('C1')->setValue('Value 2');

        $result = $this->paramlock->getall_data();
        if ($result != NULL) {
            $i = 1;
            foreach ($result as $row) {
                $i++;

                $objSheet->getCell('A' . $i)->setValue($row['IDParam']);
                $objSheet->getCell('B' . $i)->setValue($row['Val1']);
                $objSheet->getCell('C' . $i)->setValue($row['Val2']);
            }
            ...
            ob_end_clean();
            $objWriter->save($path_file . "paramlock_attendance" . $ext);
            $data = file_get_contents($path_file . "paramlock_attendance" . $ext);
            force_download("paramlock_attendance" . $ext, $data);
        }
    }
