...
class Home extends CI_Controller {
   ...
   function index() {
    ...
    function getrange_date() {
        $from = date('Y-m-d', strtotime($this->input->post('f03')));
        $until = date('Y-m-d');
        ...
        $resultholiday = $this->incomplete->get_holiday($from, $until);
        ...
        $jumlahhari = $sumdays - $jumlahlibur;

        $mesg = 'parsing data success';
        $valid = 'true';
        $json = '{ "sumday":"' . $jumlahhari . '",
                    "mesg":"' . $mesg . '",
		   "valid":"' . $valid . '"' .
                '}';
        echo $json;
