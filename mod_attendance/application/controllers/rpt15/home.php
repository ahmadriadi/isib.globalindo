...
class Home extends CI_Controller {
   ...
   function autocomplete_employee() {
    ...
    function reportdata($group, $from, $until, $name = '') {
        ...
        if ($cekdata !== 'empty') {            
            ...
            $data['url_excel'] = site_url('rpt15/home/excel' . '/' . $group . '/' . $from . '/' . $until . '/' . $name);
   ...
   function excel($group, $fromdate, $untildate, $nip) {
        ...
        $objSheet->setTitle('absence field');
        ...
        $objSheet->getCell('I1')->setValue('RTC');
	...
	$lastemp_unit = '';
                ...
                if ($Lastemp_IDEmployee !== $row['IDEmployee'] AND $lastemp_FullName !== $row['FullName'] && $n > 0) {
                    ...
                    $i++;
                     if($lastemp_unit =='SECURITY'){  
                        $objSheet->getStyle('A' . $counter . ':I' . $counter)->applyFromArray($style);
                        ...
                        $objSheet->getCell('I' . $counter)->setValue($RTC_Count);
                        $A_Count = $SP_Count = $OL_Count = $SN_Count = $ALD_Count = $RTC_Count= 0;    
                    }else{                                
                        $objSheet->getCell('A' . $counter)->setValue($i);
                        $objSheet->getCell('B' . $counter)->setValue("'" . $Lastemp_IDEmployee);
        ...
        $objSheet->getStyle('A1:I' . $counter)->getBorders()->
        ...
        ob_end_clean();
        $objWriter->save($path_file . "summary_absence_field" . $ext);
        $data = file_get_contents($path_file . "summary_absence_field" . $ext);
        force_download("summary_absence_field" . "-" . date('d-m-Y h:i') . $ext, $data);
    }
}
