...
//=============PASSIVE OR ACTIVE=======================
    ...
    function send_email_internal($subject = '', $message = '') {
        ...
        $this->email->from('admintec@trias.loc', 'SYSTEM');
...
function exportdata($param,$group,$loc,$ext = '.xlsx', $path_file = '/tmp/') {  
        ...
        $objSheet->getCell('AC1')->setValue('ID Family Certificate');
        $objSheet->getCell('AD1')->setValue('NPWP');
        $objSheet->getCell('AE1')->setValue('KPJ');      
        $objSheet->getCell('AF1')->setValue('Citized Card(KTP)');
        $objSheet->getCell('AG1')->setValue('ID Married Couple');
            ...
            foreach ($array as $row) {
		     ...
		     $objSheet->getStyle('A' . $i . ':AU' . $i)->getFill()->applyFromArray($pembatas); 	             
               ...
               // $objSheet->getCell('G' . $i)->setValue("'".$row['IDEmployeeParent']);
                ...
                $objSheet->getCell('AC' . $i)->setValue("'".$row['NoFamCert']);
                $objSheet->getCell('AD' . $i)->setValue("'".$row['NoNPWP']);
                $objSheet->getCell('AE' . $i)->setValue("'".$row['NoKPJ']);
                $objSheet->getCell('AF' . $i)->setValue("'".$row['NoKTP']);
                $objSheet->getCell('AG' . $i)->setValue("'".$row['CoupleKTP']);
