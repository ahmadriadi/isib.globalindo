...
function exportdata($param,$group,$loc,$ext = '.xlsx', $path_file = '/tmp/') {  
        ...
        $objSheet->getCell('AI1')->setValue('Number of Children');
        $objSheet->getCell('AJ1')->setValue('First Child');
        $objSheet->getCell('AK1')->setValue('Second Child');
        $objSheet->getCell('AL1')->setValue('Couple Name');
        $objSheet->getCell('AM1')->setValue('Blood Type');
        $objSheet->getCell('AN1')->setValue('Telphone');
        $objSheet->getCell('AO1')->setValue('Handphone');             
        $objSheet->getCell('AP1')->setValue('Address (Current)');      
        $objSheet->getCell('AQ1')->setValue('Address (KTP)');      
        $objSheet->getCell('AR1')->setValue('Work Experience');      
        $objSheet->getCell('AS1')->setValue('Resign Date');      
        $objSheet->getCell('AT1')->setValue('Reason Explain Resignation');
        $objSheet->getCell('AU1')->setValue('Employee Status');
        $objSheet->getCell('AV1')->setValue('Status');
        $objSheet->getCell('AW1')->setValue('Note');
        ...
        if ($result != NULL) {
            ...
            foreach ($array as $row) {
                ...
                if ($laststatus != $row['Status'] && $point > 1) {
                    $i++;      
		     $objSheet->getStyle('A' . $i . ':AU' . $i)->getFill()->applyFromArray($pembatas); 	             
	        ...
	        $redu = $this->pbl->get_lastedu($row['IDEmployee']);
               ...
               // $objSheet->getCell('G' . $i)->setValue("'".$row['IDEmployeeParent']);
                ...
                $objSheet->getCell('AI' . $i)->setValue($row['NumberChildren']);
                $objSheet->getCell('AJ' . $i)->setValue($row['FirstChild']);
                $objSheet->getCell('AK' . $i)->setValue($row['SecondChild']);
                $objSheet->getCell('AL' . $i)->setValue($row['CoupleName']);
                $objSheet->getCell('AM' . $i)->setValue($row['BloodType']);
                $objSheet->getCell('AN' . $i)->setValue("'".$row['NoTelp']);
                $objSheet->getCell('AO' . $i)->setValue("'".$row['NoHp']);              
                $objSheet->getCell('AP' . $i)->setValue($row['LiveAddress']);
                $objSheet->getCell('AQ' . $i)->setValue($row['KTPAddress']);
                $objSheet->getCell('AR' . $i)->setValue($row['WorkExperience']);
                $objSheet->getCell('AS' . $i)->setValue($row['ResignDate']);
                $objSheet->getCell('AT' . $i)->setValue($row['ReasonResign']);
                $objSheet->getCell('AU' . $i)->setValue($row['EmployeeStatus']);
                $objSheet->getCell('AV' . $i)->setValue($row['Status']);
                $objSheet->getCell('AW' . $i)->setValue($row['Note']);
               
            }

            // create some borders
            // first, create the whole grid around the table
            $objSheet->getStyle('A1:AW' . $i)->getBorders()->
                    getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
            $objSheet->getStyle('A1:AW' . $i)->getBorders()->
                    getOutline()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
            $objSheet->getStyle('A1:AW1')->getBorders()->
