...
//=============PASSIVE OR ACTIVE=======================
	...
	//$cc =array('denny@triasindrasaputra.loc','riadi@triasindrasaputra.loc','okierie@triasindrasaputra.loc'); 
        ...
        $this->email->from('donotreply@trias.loc', 'SYSTEM');
        $this->email->to('alluser@trias.loc');
        $this->email->cc('doris@trias.loc');
...
function exportdata($param,$group,$loc,$ext = '.xlsx', $path_file = '/tmp/') {  
        ...
        $objSheet->getCell('Y1')->setValue('Education Majors');
        $objSheet->getCell('Z1')->setValue('Marital Status');
        $objSheet->getCell('AA1')->setValue('Marriage Certificate');
        $objSheet->getCell('AB1')->setValue('Family Member Certificate');
        ...
        $objSheet->getCell('AG1')->setValue('Number of Children');
        $objSheet->getCell('AH1')->setValue('First Child');
        $objSheet->getCell('AI1')->setValue('Second Child');
        $objSheet->getCell('AJ1')->setValue('Couple Name');
        $objSheet->getCell('AK1')->setValue('Blood Type');
        $objSheet->getCell('AL1')->setValue('Telphone');
        $objSheet->getCell('AM1')->setValue('Handphone');             
        $objSheet->getCell('AN1')->setValue('Address (Current)');      
        $objSheet->getCell('AO1')->setValue('Address (KTP)');      
        $objSheet->getCell('AP1')->setValue('Work Experience');      
        $objSheet->getCell('AQ1')->setValue('Resign Date');      
        $objSheet->getCell('AR1')->setValue('Reason Explain Resignation');
        $objSheet->getCell('AS1')->setValue('Employee Status');
        $objSheet->getCell('AT1')->setValue('Status');
        $objSheet->getCell('AU1')->setValue('Note');
            ...
            foreach ($array as $row) {
		     ...
		     $objSheet->getStyle('A' . $i . ':AU' . $i)->getFill()->applyFromArray($pembatas); 	             
               ...
               // $objSheet->getCell('G' . $i)->setValue("'".$row['IDEmployeeParent']);
                ...
                $objSheet->getCell('X' . $i)->setValue($row['IDEducation']);
                $objSheet->getCell('Y' . $i)->setValue($row['IDMajors']);
                $objSheet->getCell('Z' . $i)->setValue($row['MaritalStatus']);                   
                $objSheet->getCell('AA' . $i)->setValue($row['MarriageCertificate']);
                $objSheet->getCell('AB' . $i)->setValue($row['FamilyMemberCertificate']);
                ...
                $objSheet->getCell('AG' . $i)->setValue($row['NumberChildren']);
                $objSheet->getCell('AH' . $i)->setValue($row['FirstChild']);
                $objSheet->getCell('AI' . $i)->setValue($row['SecondChild']);
                $objSheet->getCell('AJ' . $i)->setValue($row['CoupleName']);
                $objSheet->getCell('AK' . $i)->setValue($row['BloodType']);
                $objSheet->getCell('AL' . $i)->setValue("'".$row['NoTelp']);
                $objSheet->getCell('AM' . $i)->setValue("'".$row['NoHp']);              
                $objSheet->getCell('AN' . $i)->setValue($row['LiveAddress']);
                $objSheet->getCell('AO' . $i)->setValue($row['KTPAddress']);
                $objSheet->getCell('AP' . $i)->setValue($row['WorkExperience']);
                $objSheet->getCell('AQ' . $i)->setValue($row['ResignDate']);
                $objSheet->getCell('AR' . $i)->setValue($row['ReasonResign']);
                $objSheet->getCell('AS' . $i)->setValue($row['EmployeeStatus']);
                $objSheet->getCell('AT' . $i)->setValue($row['Status']);
                $objSheet->getCell('AU' . $i)->setValue($row['Note']);
