...
class Home extends CI_Controller {
   ...
   function autocomplete_employee() {
    ...
    function excel($group, $fromdate, $untildate, $nip) {
        ...
        //$objSheet->getCell('J6')->setValue('DEDUC HOUR SUM');
                    ...
                    if ($prindata == 'yes') {
                        ...
                        if (($row['Position'] == 'DIRECTOR') OR ($row['Position'] == 'ASSISTANT DIRECTOR') OR ($row['Position'] == 'MANAGER')) {
                            ...
                            if ($lp == 'empty' and $shift !== '2-1' and $flag=='empty') {
                                ...
                                if ($nip != $last_idemployee) {
                                    $TotLH = $deducsum = 0;
                                }
                                $n++;
                                $i++;
                                $TotLH = $TotLH + $tmplt;
                                $deducsum = $deducsum + $deducstatus;
                                ...
                                //$objSheet->getCell('H' . $i)->setValue($this->decimaltominutes($TotLH));
                                //$objSheet->getCell('H' . $i)->setValue($deducstatus . ' Hour');
				 $objSheet->getCell('H' . $i)->setValue($deducstatus);
                                //$objSheet->getCell('J' . $i)->setValue($deducsum . ' Hour');
                                $last_idemployee = $nip;
                            } else {
