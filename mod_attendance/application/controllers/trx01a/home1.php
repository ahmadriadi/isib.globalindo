...
class Home extends CI_Controller {
 ...
 function index() {
		...
		$data['accessbutton']='true';
                $result = $this->overtime->overtimedata_hrd($fromdate, $untildate);
            } else if ($role == '1') {
                $parameter = $rowparam->ParamValue;
                if ($parameter == $iduser) {
		    $data['accessbutton']='true';	
    ...
    function addpost() {
		...
		'FlagInput' => 'hrd',
            ...
            if (($sumhour <= 0) OR ($sumhour >= 20)) {
                ...
                $resultdata = $this->overtime->checkdata($f02,$presence,$timein);               
 ...
 $f = $this->session->userdata('datefrom');
	 ...
	 $parameter = $rowparam->ParamValue;
        ...
        if ($checkdata1 == 'exist' and $checkdata2 == 'exist') {
            ...
            if ($role == '2') {
                $result = $this->overtime->overtimedata_hrd($f, $u);
            } else if ($role == '1') {
                $parameter = $rowparam->ParamValue;
                if ($parameter == $iduser) {
                    $result = $this->overtime->overtimedata_hrd($f, $u);
