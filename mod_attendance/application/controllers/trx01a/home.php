...
class Home extends CI_Controller {
 ...
 function send_email_eksternal($subject = '', $message = '') {
        $email_config = Array(
            'protocol'  => 'smtp',
            'smtp_host' => 'ssl://smtp.gmail.com',
            'smtp_port' => '465',
            'smtp_user' => 'testerdev72@gmail.com',
            'smtp_pass' => '02197406932',
            'mailtype'  => 'html',
            'starttls'  => true,
            'newline'   => "\r\n",
            'crlf'   => "\r\n"
        ...
        $this->email->from('testerdev72@gmail.com', 'TIS SYSTEM');
        $this->email->to('info.ahmadriadi@gmail.com');
        //$this->email->cc($cc);
 ...
 function send_email_internal($subject = '', $message = '') {
	...
	//$cc =array('denny@triasindrasaputra.loc','riadi@triasindrasaputra.loc','okierie@triasindrasaputra.loc'); 
        ...
        $this->email->initialize($email_config);
        //$this->email->clear(TRUE);
        $this->email->from('donotreply@triasindrasaputra.loc', 'CHECKING ERROR DATA');
        $this->email->to('riadi@triasindrasaputra.loc');
        $this->email->cc('riadi@triasindrasaputra.loc');
        $this->email->subject($subject);
        $this->email->message($message);
        if (!$this->email->send()) {
            return FALSE;
 ...
 function index() {
	...
	$data['default']['location'][2]['value'] = '2';
		...
		$iduser = $this->session->userdata('sess_userid');
                ...
                if($role == '0' and $position == 'DIRECTOR' or $position == 'DIREKTUR' or $position == 'ASS. MANAGER' or $position == 'KOMISARIS' or $position == 'PROJECT TEAM LEADER' or $position =='MANAGER' or $position =='ASSISTANT MANAGER' or $position == 'SUPERVISOR' or $position == 'ASSISTANT SUPERVISOR' or $param=='Y'){
	    ...
	    $subject="CHECK USER OVERTIME";        
             $message = 'User :'.$iduser.'<br/> Keadaan Akses Login :'.$checkdata1. ' <br/> Keadaan Akses Menu :'.$checkdata2.' <br/> Role Login :'.$role.' <br/> Posisi Jabaran : '.$position
                   .' <br/> Keadaan Data Overtime :'.$checkresult;               
              
	       //$this->send_email_eksternal($subject, $message);	
   ...
   function dataovertime() {
	...
	$rowpersonal = $this->employee->get_position($iduser);
        ...
        if ($check1 == 'exist' and $check2 == 'exist') {
            $role = $rowlogin->Role;
	    $position = $rowpersonal->Position;	
            if ($role == '1' or $role == '2' or $position=='SUPERVISOR' or $param=='Y') {
   ...
   function autocomplete_employee() {
	...
	if($dept=='14' or $role=='1' or $role=='2'){
            $data['inputdata'] = 'open'; 
	...
	*/
    ...
    function addpost() {
	    ...
	    $f04temp = $this->anti_xss($this->input->post('f04'));		
	    /*            
	  ...
	  */	
	   ...
	   $entrydate =$this->anti_xss($this->input->post('entrydate'));
            if($entrydate=='form'){
                $f04 = $f04temp;
            ...
            $f06B = $this->anti_xss($this->input->post('f06B'));
            $h2 = substr($f06B, 0, 2);
            $m2 = substr($f06B, 2, 2);
            $out = $h2 . ':' . $m2;
		...
		'FlagInput' => 'hrd',
            ...
            if (($sumhour <= 0) OR ($sumhour >= 20)) {
                ...
                if($check =='exist'){                      
                     ...
                     $txt = ' Presence Date '.$f04.' Overtime Hour:' . $f05 . ' to ' . $f06 . '  =' . $sumhour . ' Hour';
                     $val = 'true';
                }               
    ...
    function editpost() {
	    ...
	    $entrydate =$this->anti_xss($this->input->post('entrydate'));
            ...
            if (($sumhour <= 0) OR ($sumhour >= 20)) {
                ...
                $mesg = 'Presence Date '.$f04.' Overtime Hour:' . $f05 . ' to ' . $f06 . '  =' . $sumhour . ' Hour';
                $valid = 'true';
                $err_f01 = '';
                $err_f02 = '';
                $err_f04 = '';
