...
class Home extends CI_Controller {
    ...
    function presencedata($group, $f, $u) {
        $dfrom = date('Y-m-d', strtotime($f));
        $duntil = date('Y-m-d', strtotime($u));

        $result = $this->report->check_allpresence($group, $dfrom, $duntil);
    ...
    function iframedata($group, $from, $until) {
        $dfrom = date('Y-m-d', strtotime($from));
        $duntil = date('Y-m-d', strtotime($until));
        $data['url'] = site_url('rpt01/home/reportdata/' . $group . '/' . $dfrom . '/' . $duntil);
        $this->load->view('rpt01/iframe', $data);
    }

    function reportdata($group, $from, $until) {
        ...
        $resuldata = $this->report->report_summary($from, $until,$group);
    ...
    function excel($group, $fromdate, $untildate) {
        error_reporting(0);
        $ext = '.xlsx';
        $path_file = '/tmp/';
        ...
        $result = $this->report->report_summary($fromdate, $untildate,$group);
