...
function accept(){
    ...
    if (jmlbox == 1){
        if (submitted[0] == val){
            loading();
            $.ajax({
                url     : ROOT.base_url+'/mod_attendance/index.php/trx02/home/confirm',
                data    : "idlpermit=<?php echo $idlpermit;?>&stat=1",// rencana ada via web dan email. yang lain di sini via=web
                type    : "post",
                success : function (data){
                    bootbox.alert("Leave Permit accepted! <br> Thanks for your confirmation", function (){
                        bootbox.hideAll();
                    });
                    reloadview();
                }
        ...
        if (submitted[0] == val){
            bootbox.prompttextarea("You are going to reject this Leave Permit request.</isi>Please write your reason here :", function (r){
                if (r === null){
                    ...
                    $.ajax({
                        url     : ROOT.base_url+'/mod_attendance/index.php/trx02/home/confirm',
                        data    : "idlpermit=<?php echo $idlpermit;?>&stat=2&reason="+r,
                        type    : "post",
//                        dataType: "html",
                        success : function (data){
    //                        bootbox.alert(data);
