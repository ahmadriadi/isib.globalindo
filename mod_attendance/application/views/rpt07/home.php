...
<div class="box-generic">
    ...
    <div class="tabsbar">
        <ul>
            <li class="glyphicons tag active"><a href="#presencejurnal" data-toggle="tab"><i></i>REPORT LEAVEPERMIT<strong></strong></a></li>
                ...
                <div class="widget-body"> 
                    <div class="row-fluid">
                        ...
                        <div class="span6">
                            ...
                            <!-- Group -->
                            <div class="control-group">
                                <label class="control-label" for="f05">Necessity</label>
                                <div class="controls">
                                    <?php
                                    $no = 'A';
                                    foreach ($default['f05'] as $row) {
                                        ?>  
                                        <input id="f05<?php echo $no ?>" name="f05" type="radio" 
                                               value="<?php echo (isset($row['value'])) ? $row['value'] : ''; ?>"
                                               <?php echo (isset($row['checked'])) ? $row['checked'] : ''; ?> >
                                        <?php echo (isset($row['display'])) ? $row['display'] : ''; ?> <br/>
                                        <?php
...
<script>
	...
	/*
        var url_suggest = '<?php echo site_url('rpt07/home/suggest_employee'); ?>';
        $("#f03").autocomplete({
            minLength: 2,
	...
	*/
	 ...
	 $.ajax({
	    url: '<?php echo site_url('rpt07/home/autocomplete_employee'); ?>',
	    dataType: 'json',
	    success: function(d) {
		var data = d;
		$('#f03').typeahead({
        ...
        process.click(function() {
            var url_presence = '<?php echo site_url('rpt07/home/presencedata') ?>';
            var fromdate = $("#f01").val();
            var untildate = $("#f02").val();
            var nip = $("#f03").val();
            var group = $('input:radio[name=f04]:checked').val();
            var necessity = $('input:radio[name=f05]:checked').val();
            var content = $("#content .innerLR");
            var site = 'mod_attendance/index.php/rpt07/home/iframedata/' + fromdate + '/' + untildate + '/' + group + '/' + necessity+'/'+nip;
            var url = ROOT.base_url + site;

            var url_post = url_presence + '/' + fromdate + '/' + untildate + '/' + group + '/' + necessity+'/'+nip;
