...
<script type="text/javascript">
    ...
    $(document).ready(function()
    {
        ...
        var url_suggest = '<?php echo site_url('trx07/home/suggest_employee'); ?>';
        var url_charge = '<?php echo site_url('trx07/home/suggest_charge'); ?>';
        var url_reference = '<?php echo site_url('trx07/home/reference_leave'); ?>';

        var flag = '<?php echo $flag; ?>';


        $.ajax({
	    url: '<?php echo site_url('trx07/home/autocomplete_employee'); ?>',
	    dataType: 'json',
	    success: function(d) {
		var data = d;
		$('#f02').typeahead({
			...
			$('#f01').val(s[1]); /* for add item */                        
                          $.ajax({
                            type: "POST",
                            dataType: "json",
                            url: "<?php echo site_url('trx07/home/rest_leave') ?>",
                            data: "nip=" + s[1],
                            success:
                                    function(data, text)
                                    {
                                        if (data.valid == 'true') {
                                            $("#amountleave").html(" Amount Leave " + data.restleave + " Days");
     ...
     $.ajax({
	    url: '<?php echo site_url('trx07/home/autocomplete_change'); ?>',
	    dataType: 'json',
	    success: function(d) {
		var data = d;
		$('#f03').typeahead({
       ...
       /*
        ...
        if (flag == 'add') {
            $('#f02').focusout(function() {
                var nip = $("#f01").val();
                var key = 'add';
                $.ajax({
                    type: "POST",
                    url: url_reference,
                    data: "nip=" + nip + "&key=" + key,
                    success: function(data) {
                        $("#f07").html(data);
