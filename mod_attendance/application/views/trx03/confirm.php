...
<?php 
if ($dticp > 0){
    echo "$('#acpticp').prop('disabled',true);";
}
if ($dticp == 0){
...
//    alert(notfor);
    ...
    if (jmlbox == 1){
        if (submitted[0] == val){
            loading();
            $.ajax({
                url     : ROOT.base_url+'/mod_attendance/index.php/trx03/home/confirm',
                data    : "idincomplete=<?php echo $idincomplete;?>&stat=1&notfor="+notfor,// rencana ada via web dan email. yang lain di sini via=web
                type    : "post",
                success : function (data){
                    bootbox.alert("Overtime accepted! <br> Thanks for your confirmation", function (){
                        bootbox.hideAll();
                    });
                    reloadview();
                },
...
function reject(){
    ...
    if (jmlbox == 1){
        if (submitted[0] == val){
            bootbox.prompttextarea("You are going to reject this Leave Permit request.</isi>Please write your reason here :", function (r){
                if (r === null){
                    ...
                    $.ajax({
                        url     : ROOT.base_url+'/mod_attendance/index.php/trx03/home/confirm',
                        data    : "idincomplete=<?php echo $idincomplete;?>&stat=2&reason="+r+"&notfor="+notfor,
                        type    : "post",
//                        dataType: "html",
                        success : function (data){
                            bootbox.alert(data);
...
<div id="containernya" >
    ...
    <div style="margin-top: 10px;">
        <button id="acpticp" onclick="accept()" class="btn btn-small btn-success">Accept</button>
        <button id="rjcticp" onclick="reject()" class="btn btn-small btn-danger">Reject</button>
    </div>
</div>
