...
class Report_model extends CI_Model {
	...
	$this->_group = 'isib_attendance.r05jobgroup';
        ...
        if ($checknip == 'exist') {
            if($job !=='ALL'){
               $where = " WHERE a.PresenceDate BETWEEN '$fromdate' AND '$untildate' AND a.IDEmployee ='$nip' AND b.IDJobGroup ='$job' AND b.DeleteFlag='A' AND a.DeleteFlag='A' AND a.ConfirmFlag='1'"; 
                ...
                $where = " WHERE a.PresenceDate BETWEEN '$fromdate' AND '$untildate' AND a.IDEmployee ='$nip'  AND b.DeleteFlag='A' AND a.DeleteFlag='A' AND a.ConfirmFlag='1'"; 
            }
            ...
            if($job !=='ALL'){
              ...
              $where = " WHERE a.PresenceDate BETWEEN '$fromdate' AND '$untildate' AND b.DeleteFlag='A' AND a.DeleteFlag='A' AND a.ConfirmFlag='1' ";
    ...
    function report_sum_absen($fromdate, $untildate, $group, $nip) {
        ...
        if ($checknip == 'exist') {
            ...
            if($group !=='ALL'){
                ...
                $where = "  WHERE   
                            (a.PresenceDate BETWEEN '$fromdate' AND '$untildate' AND b.DeleteFlag='A') AND
                            (b.IDEmployee ='$nip' AND b.IDJobGroup NOT IN('ST')) AND
                            ((b.Status ='A') OR (b.Status ='P' AND ResignDate >='$fromdate')) AND 
                            ( (a.PresenceDate >= b.HireDate) AND (a.PresenceDate <= '$untildate') )
                    ";
            }
...
*/
    ...
    function report_absence($fromdate, $untildate, $group, $nip) {
        ...
        if ($checknip == 'exist') {            
            if($group !=='ALL'){
                 ...
                 $where = "WHERE   a.PresenceDate BETWEEN '$fromdate' AND '$untildate' AND b.Status='A'  AND a.IDEmployee='$nip' AND b.DeleteFlag='A'";
            }           
        } else {
             if($group !=='ALL'){
                  $where = "WHERE   a.PresenceDate BETWEEN '$fromdate' AND '$untildate' AND b.Status='A' AND  b.IDJobGroup ='$group' AND b.DeleteFlag='A'";
             }else{
                  $where = "WHERE   a.PresenceDate BETWEEN '$fromdate' AND '$untildate' AND b.Status='A' AND b.DeleteFlag='A'";
             }
   ...
   function report_incomplete($fromdate, $untildate, $group, $nip) {
        ...
        $query = "SELECT a.*,b.FullName,b.IDJobGroup,c.Description rDescription,d.GroupName
                 FROM $this->_presence a
                 LEFT JOIN $this->_personal b    
                 ON a.IDEmployee = b.IDEmployee
                 LEFT JOIN $this->_typepresence c
                 ON a.Description = c.IDType 
                 LEFT JOIN $this->_group d
                 ON b.IDJobGroup = d.IDJobGroup   
                 $where
                 ORDER BY b.IDJobGroup DESC,b.FullName ASC,PresenceDate ASC    
                ";
        $result = $this->_db_at->query($query);
        if ($result->num_rows() > 0) {
            return $result->result_array();
    ...
    function report_incomplete_form($fromdate, $untildate, $group, $nip) {
        ...
        $sql = "SELECT
               ...
               $where
                   ORDER BY b.IDJobGroup DESC,b.FullName ASC,a.IncompleteDate ASC
 ...
 function report_travel($from, $until, $nip) {
  ...
  function report_sickness($from, $until, $group, $nip) {
        ...
        $sql = "SELECT a.*,b.FullName,b.IDJobGroup
                ...
                ON a.IDEmployee = b.IDEmployee
                $where
                 ORDER BY b.IDJobGroup DESC,b.FullName ASC, a.TglCutiDari ASC        
                ";
 ...
 function get_leavepermit($idemployee,$date){
   ...
   function report_countemployee_active($date){
                $query = "
                        SELECT a.*,b.DescStructure FROM $this->_personal a
                        LEFT JOIN $this->_dept b ON b.IDStructure = a.IDDepartement     
                        WHERE  
                        a.HireDate <='$date' AND
                        a.IDEmployee NOT IN (Select IDEmployee FROM $this->_personal WHERE ResignDate <='$date') 
                            
                        ORDER BY a.IDJobGroup DESC, a.FullName ASC
                        ";
           $result = $this->_db_emp->query($query);     
           if ($result->num_rows() > 0) {
                return $result->result_array();
    ...
    function report_countemployee_passive($date){
                $query = "
                        SELECT  a.*,'Resign' AS TITLE,b.DescStructure
                        FROM    $this->_personal a
                        LEFT JOIN $this->_dept b ON b.IDStructure = a.IDDepartement      
                        WHERE  a.ResignDate <= '$date' 
                        ORDER BY a.IDJobGroup DESC, a.FullName ASC     
                        ";
