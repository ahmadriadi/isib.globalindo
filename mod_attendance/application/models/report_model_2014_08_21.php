...
//		return $query->row();
    ...
    function report_late($fromdate, $untildate, $group, $nip) {
        $checknip = ($nip == '' or $nip == null) ? 'empty' : 'exist';
        if ($checknip == 'exist') {
            $where = " WHERE   a.PresenceDate BETWEEN '$fromdate' AND '$untildate' AND  b.IDJobGroup ='$group'  AND b.IDEmployee='$nip'";
        } else {
 ...
 function report_leave($fromdate,$untildate,$nip) {
        $checknip = ($nip == '' or $nip == null) ? 'empty' : 'exist';
        if ($checknip == 'exist') {
            $where = "WHERE a.PresenceDate BETWEEN '$fromdate' AND '$untildate' AND a.IDEmployee='$nip'";
        } else {
            $where = "WHERE a.PresenceDate BETWEEN '$fromdate' AND '$untildate'";
        }
       ...
       if($group !=='ST'){
        ...
        if ($count == 0) {
            ...
            $query = "
                ...
                FROM    $this->_personal
                WHERE   HireDate >= '$fromdate' AND HireDate <= '$untildate'  AND $jobgroup
                     
                UNION
                SELECT  *, DATEDIFF(ResignDate,'$fromdate') AS Priv
                FROM    $this->_personal
                WHERE   ResignDate >= '$fromdate' AND ResignDate <= '$untildate' AND $jobgroup
