...
class Home extends CI_Controller {
    ...
    function index() {
        ...
        $data['iddept'] = $rowpersonal->IDDepartement;
     ...
     function presencedata($from,$until,$iddept) {
        ...
        $result = $this->report->report_leave($dfrom, $duntil,$iddept);
    ...
    function iframedata($from, $until,$iddept) {
        $dfrom = date('Y-m-d', strtotime($from));
        $duntil = date('Y-m-d', strtotime($until));
        $data['url'] = site_url('rpt04/home/reportdata/' . $dfrom . '/' . $duntil . '/' . $iddept);
        $this->load->view('rpt04/iframe', $data);
    }

    function reportdata($from,$until,$iddept) {
        ini_set('memory_limit', '-1'); // for unlimited size from file excel	
        $resuldata = $this->report->report_leave($from,$until,$iddept);       
        $cekdata = ($resuldata !== null) ? $resuldata : 'empty';
        if ($cekdata !== 'empty') {
            $data['fromdate'] = $from;
            $data['untildate'] = $until;
            $data['url_excel'] =  site_url('rpt04/home/excel')."/".$from.'/'.$until;
            $data['resultdata'] = $resuldata;
            $this->load->view('rpt04/report', $data);
        }
    }

    function excel($fromdate,$until,$iddept) {
        ini_set('memory_limit', '-1'); // for unlimited size from file excel	
        $result = $this->report->report_leave($fromdate,$until,$iddept);         
         // Start Excel
        $ext = '.xlsx';
        $path_file = '/tmp/';
