...
class Home extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->userid = $this->session->userdata("sess_userid");
    }
    ...
    function autocomplete_employee() {
        $rpst = $this->employee->get_position($this->userid);
        $param = $this->employee->getparam_rptdetail($this->userid);
	
        $position = $rpst->Position;
        $dept = $rpst->Department;
        if($position=='DIRECTOR' or $position=='COMISSIONER'){
              $result = $this->employee->get_all($q);
        }else if($position=='MANAGER' or $position=='ASSISTANT MANAGER' or $param=='exist'){
              $result = $this->employee->get_for_man($q,$dept,$this->userid);  
        }else if($position=='SUPERVISOR'){
              $result = $this->employee->get_for_spv($q,$dept);  
        }else{
             $result = $this->employee->get_for_user($q,$this->userid);  
        }
    ...
    function suggest_employee() {
        ...
        if($position=='DIRECTOR' or $position=='COMISSIONER'){
              $result = $this->employee->get_all($q);
        }else if($position=='MANAGER' or $position=='ASSISTANT MANAGER'){
              $result = $this->employee->get_for_man($q,$dept,$this->userid);  
        }else if($position=='SUPERVISOR'){
              $result = $this->employee->get_for_spv($q,$dept);  
        }else{
    ...
    function iframedata($nip, $from, $until) {
        ...
        $data['url'] = site_url('rpt01/home/reportdata/' . $nip . '/' . $dfrom . '/' . $duntil);
               ...
               // $Late = $row['LateHour'];
                ...
                if ($row['Description'] == 'P') {
                   ...
                   if ($actualhour <= 4) { 
                    ...
                    $PLW_Count+= 1;
                    $plw = 1;
                }elseif ($row['Description'] == 'SN' or $row['Description'] == 'LSN') {
                    $SN_Count+= 1;
                    $sn = 1;
                }  elseif ($row['Description'] == 'AL' or $row['Description'] == 'OL') {
                    $L_Count+= 1;
                    $l = 1;
