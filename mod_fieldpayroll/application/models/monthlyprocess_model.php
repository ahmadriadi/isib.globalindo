...
class Monthlyprocess_model extends CI_Model {
    ...
    function get_loaninterest($id){
       ...
       function update_loaninterest($id,$record){
        $this->_db->where('IDRecord',$id);
        $this->_db->update($this->_interest,$record);
               
    } 	

    function get_additionleave($posting){
       $sql = "SELECT * FROM $this->_additional_leave WHERE
               PostingDate='$posting' 
         ";
    ...
    function get_paraminsentive($nip){
        $this->_db->where('DeleteFlag','A');
        $this->_db->where('IDEmployee',$nip);
        $result = $this->_db->get($this->_paraminsentive);
        if($result->num_rows()>0){
            return 'exist';
        }else{
            return 'empty';
   ...
   function delete_late($posting, $nip) {
        $chekdata = ($nip == '' or $nip == null) ? 'empty' : 'exist';
        if ($chekdata == 'exist') {
            $where = "WHERE  PostingDate ='$posting' AND IDEmployee ='$nip'";
        } else {
        ...
        $sql = "DELETE FROM $this->_late $where ";
        $this->_db->query($sql);
    }
    ...
    function getperiode_loanmanual($from, $until, $nip) {
        ...
        if ($chekdata == 'exist') {
            $where = "  WHERE
                        ...
                        (b.Status='A' AND b.DeleteFlag='A' OR (b.Status='P' AND (b.ResignDate BETWEEN '$from' AND '$until')))
                    ";
        } else {
            $where = "  WHERE                       
    ...
    function getdata_presence($from, $until, $nip) {
        ...
        $sql =
                "SELECT 
            a.IDEmployee,
            b.FullName,b.IDJobGroup,b.HireDate,b.ResignDate,b.IDLocation,
            a.PresenceDate,a.WorkDay,a.ActualIn,a.ActualOut,a.ManualIn,a.ManualOut,
            a.Description, a.Necessity,
	    c.DailySalary,c.MonthlySalary,c.Insurance,c.BPJS  
    ...
    function getdata_overtime($from, $until, $nip) {
        ...
        if ($chekdata == 'exist') {
            $where = "  
                    WHERE
                    a.IDEmployee='$nip' AND
                    a.DeleteFlag ='A' AND a.ConfirmFlag='1' AND    
                    a.PresenceDate BETWEEN '$from' AND '$until' AND
                    (b.IDJobGroup IN ('LT','LK','HL','MAG','OS')) AND
                    (b.Status='A' AND b.DeleteFlag='A' OR (b.Status='P' AND (b.ResignDate BETWEEN '$from' AND '$until')))
