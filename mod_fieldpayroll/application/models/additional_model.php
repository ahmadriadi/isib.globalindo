...
class Additional_model extends CI_Model {
    ...
    public function __construct() {
        ...
        $this->_table      = 't03addition';
        $this->_employee   = 'triasnet_employee.m01personal';
        $this->_kelebihanbpjs   = 'tmpkelebihanbayarbpjs';
       
    }
    
    
    function  gettemp_additionbpjs(){
         return $this->_db->get($this->_kelebihanbpjs)->result_array();
        
    }
          
    
 function dataaddition($from,$until){            
      $a = $this->_table;   
  ...
  function checkdata($nip,$posting,$code){
      $this->_db->where('DeleteFlag','A');
      $this->_db->where('FlagEntry',$code);	
