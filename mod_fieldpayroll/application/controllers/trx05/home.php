   ...
   function index() {	        
    ...
    function resultdeduction() {	
        ...
        if ($check1 == 'exist' and $check2 == 'exist') {
            $role = $rowlogin->Role;            
            if ($role == '1' or $role == '2') {
                    echo $this->deduction->datadeduction($f,$u);		
    ...
    function exportdata($g) {
        ...
        // rename the sheet
        $objSheet->setTitle('deduction report');
        ...
        $f = $this->session->userdata('datefrom');
        $u = $this->session->userdata('dateuntil');
        $result = $this->deduction->getall_data($f,$u,$g);
        if ($result != NULL) {   
            $i = 1;
            ...
            ob_end_clean();
            $objWriter->save($path_file . "deduction" . $ext);
            $data = file_get_contents($path_file . "deduction" . $ext);
            force_download("deduction" . $ext, $data);
        }
    }
