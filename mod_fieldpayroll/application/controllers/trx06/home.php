...
class Home extends CI_Controller {
    ...
    public function __construct() {        
        parent ::__construct();
        $this->load->model('manualdeduction_model', 'mdeduction');
   ...
   function index() {	        
	...
	$data['datadate']          = date('d');
        ...
        $this->load->view('trx06/home',$data);
    }

    function resultmdeduction() {	
        ...
        if ($check1 == 'exist' and $check2 == 'exist') {
            $role = $rowlogin->Role;            
            if ($role == '1' or $role == '2') {
                  echo $this->mdeduction->datadeduction($f,$u);	
  ...
  function autocomplete_employee() {
   ...
   function suggest_employee() {
    ...
    function addnew() {
	$date = $this->libfun->periode_posting_entry();	
        $data['default']['f01'] = ''; //IDEmployee
        $data['default']['f02'] = ''; //FullName
        $data['default']['f03'] = substr($date, 11, 10); //PostingDate
        $data['default']['f04'] = '';//Amount
        $query = $this->mdeduction->get_all_reference()->result();                
        $i = 0;
        foreach ($query as $r) {
        ...
        $data['url_post'] = site_url('trx06/home/addpost');        
        $this->load->view('trx06/form',$data);
        ...
        $this->form_validation->set_rules('f05', 'Paramenter', 'required');      
         if ($this->form_validation->run() == TRUE) {
            ...
            $recordlog = array(
                ...
                'controller' => site_url('trx06/home/addnew'),
                'activities' => 'add new ' . $f01
            );

            //$result = $this->mdeduction->checkdata($f01,$f03);
    ...
    function edit($id) {
        ...
        $row = $this->mdeduction->get_by_id($id);
        ...
        $data['url_post'] = site_url('trx06/home/editpost');       
        $this->load->view('trx06/form',$data);
       ...
       $this->form_validation->set_rules('f01', 'IDEmployee', 'required');
        ...
        $this->form_validation->set_rules('f05', 'Paramenter', 'required');      
         if ($this->form_validation->run() == TRUE) {
            ...
            $this->historydata($id);
            $this->mdeduction->update($id,$record);
            $this->logs->insert($recordlog);
    ...
    function exportdata($g) {
        ...
        // rename the sheet
        $objSheet->setTitle('mdeduction report');
	...
	$objSheet->getCell('C1')->setValue('Group');
        ...
        $f = $this->session->userdata('datefrom');
        $u = $this->session->userdata('dateuntil');
        $result = $this->mdeduction->getall_data($f,$u,$g);
        if ($result != NULL) {   
            $i = 1;
	    ...
	    $objSheet->getColumnDimension('G')->setAutoSize(true);     	      
            ...
            ob_end_clean();
            $objWriter->save($path_file . "mdeduction" . $ext);
            $data = file_get_contents($path_file . "mdeduction" . $ext);
            force_download("mdeduction" . $ext, $data);
        }
    }
   
  function historydata($id){
         $rowh = $this->history->getby_id_mandeduction($id); 
            $record = array(
                "PostingDate"=>$rowh->PostingDate,
                "IDEmployee"=>$rowh->IDEmployee,
                "Amount"=>$rowh->Amount,
                "Parameter"=>$rowh->Parameter,
                "FlagLoan"=>$rowh->FlagLoan,
                "Note"=>$rowh->Note,
            ...
            $this->history->insert_hismandeduction($record);
   }
