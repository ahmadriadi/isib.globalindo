...
class Home extends CI_Controller {
    ...
    public function __construct() {        
        parent ::__construct();
        $this->load->model('dailyovertime_model', 'dailyovertime');
   ...
   function index() {	        
    ...
    function resultdailyovertime() {	
        ...
        if ($check1 == 'exist' and $check2 == 'exist') {
            $role = $rowlogin->Role;            
            if ($role == '1' or $role == '2') {
                   echo $this->dailyovertime->datadailyovertime($f,$u);	
    ...
    function exportdata($g) {
        ...
        // rename the sheet
        $objSheet->setTitle('dailyovertime report');
	...
	$objSheet->getCell('C1')->setValue('Group');
        $objSheet->getCell('D1')->setValue('PostingDate');
        $objSheet->getCell('E1')->setValue('PresenceDate');
        $objSheet->getCell('F1')->setValue('OvertimeIn');
        $objSheet->getCell('G1')->setValue('OvertimeOut');
        $objSheet->getCell('H1')->setValue('OvertimeHour');
        $objSheet->getCell('I1')->setValue('OvertimeTotalHour');
        $objSheet->getCell('J1')->setValue('OvertimePerHour');
        $objSheet->getCell('K1')->setValue('PaymentStatus');
        $objSheet->getCell('L1')->setValue('DailyOvertimePayment');
        $objSheet->getCell('M1')->setValue('OvertimeIncentive');
      
        
        $f = $this->session->userdata('datefrom');
        $u = $this->session->userdata('dateuntil');
        $result = $this->dailyovertime->getall_data($f,$u,$g);
        if ($result != NULL) {   
            $i = 1;
		...
		$objSheet->getCell('C' . $i)->setValue($group);
                ...
                $objSheet->getCell('F' . $i)->setValue($row['OvertimeIn']);
                $objSheet->getCell('G' . $i)->setValue($row['OvertimeOut']);
                $objSheet->getCell('H' . $i)->setValue($row['OvertimeHour']);
                $objSheet->getCell('I' . $i)->setValue($row['OvertimeTotalHour']);
                $objSheet->getCell('J' . $i)->setValue($row['OvertimePerHour']);
                $objSheet->getCell('K' . $i)->setValue($row['PaymentStatus']);
                $objSheet->getCell('L' . $i)->setValue($row['DailyOvertimePayment']);
                $objSheet->getCell('M' . $i)->setValue($row['OvertimeIncentive']);
                
            }

            // create some borders
            // first, create the whole grid around the table
            $objSheet->getStyle('A1:M' . $i)->getBorders()->
                    getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
            $objSheet->getStyle('A1:M' . $i)->getBorders()->
                    getOutline()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
            $objSheet->getStyle('A1:M1')->getBorders()->
	    ...
	    $objSheet->getColumnDimension('M')->setAutoSize(true); 	      
            ...
            ob_end_clean();
            $objWriter->save($path_file . "dailyovertime" . $ext);
            $data = file_get_contents($path_file . "dailyovertime" . $ext);
            force_download("dailyovertime" . $ext, $data);
        }
    }
