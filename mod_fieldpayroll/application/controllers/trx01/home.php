...
class Home extends CI_Controller {
    ...
    public function __construct() {        
        parent ::__construct();
        $this->load->model('dailysalary_model', 'dailysalary');
   ...
   function index() {	        
	...
	$query = $this->employee->get_rjob_field()->result();
      ...
      function resultdailysalary() {	
        ...
        if ($check1 == 'exist' and $check2 == 'exist') {
            $role = $rowlogin->Role;            
            if ($role == '1' or $role == '2') {
                  echo $this->dailysalary->datadailysalary($f,$u);
    ...
    function exportdata($g) {
        ...
        // rename the sheet
        $objSheet->setTitle('dailysalary report');
	...
	$objSheet->getCell('C1')->setValue('Group'); 	
        $objSheet->getCell('D1')->setValue('PresenceDate');
        $objSheet->getCell('E1')->setValue('PresenceStatus');
        $objSheet->getCell('F1')->setValue('DailySalaryPayment');
        $objSheet->getCell('G1')->setValue('MontlyPayment');
        $objSheet->getCell('H1')->setValue('DailyIncentiveShift');
        $objSheet->getCell('I1')->setValue('InsurancePayment');
      
        
        $f = $this->session->userdata('datefrom');
        $u = $this->session->userdata('dateuntil');
        $result = $this->dailysalary->getall_data($f,$u,$g);
        if ($result != NULL) {   
            $i = 1;
		...
		$objSheet->getCell('C' . $i)->setValue($group);
                $objSheet->getCell('D' . $i)->setValue(date('d-m-Y',strtotime($row['PresenceDate'])));
                $objSheet->getCell('E' . $i)->setValue($row['PresenceStatus']);
                $objSheet->getCell('F' . $i)->setValue($row['DailySalaryPayment']);
                $objSheet->getCell('G' . $i)->setValue($row['MontlyPayment']);
                $objSheet->getCell('H' . $i)->setValue($row['DailyIncentiveShift']);
                $objSheet->getCell('I' . $i)->setValue($row['InsurancePayment']);
            }
	    ...
	    $objSheet->getColumnDimension('I')->setAutoSize(true); 	      
            ...
            ob_end_clean();
            $objWriter->save($path_file . "dailysalary" . $ext);
            $data = file_get_contents($path_file . "dailysalary" . $ext);
            force_download("dailysalary" . $ext, $data);
        }
    }
