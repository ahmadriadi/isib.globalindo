...
//        echo "<hr>";
    ...
    function index(){
        ...
        $checkpicket            = $this->lvm->checkpicket($this->me,$fromdate,$untildate);   
        if($checkpicket !=='empty'){
            $frompicket = $this->lvm->rowpicket($this->me,$fromdate,$untildate,'from');
            $untilpicket = $this->lvm->rowpicket($this->me,$fromdate,$untildate,'until');
            $amountpicket =   $this->long_holiday($frompicket,$untilpicket);  
        }else{            
            $amountpicket = 0;
        }       
       $data['amountpicket']  =$amountpicket;  
       $data['displayleave']  =$this->sisacuti($userid,$personal->HireDate)+$amountpicket;
       
       
       $this->load->view('trx01/home',$data);
    }
    
    function long_holiday($from, $until) {  
        $f= date('Y-m-d',  strtotime($from));
        $u= date('Y-m-d',  strtotime($until));       
        
        
        $sumdays = (strtotime($u) - strtotime($f)) / (24 * 3600) + 1;
        ...
        $resultholiday = $this->lvm->get_holidaypicket($from, $until);
        ...
        $sumofholiday = $sumdays - $jumlahlibur;
        
        //echo $sumdays .' - '.$jumlahlibur;
        //return $sumofholiday;
   ...
   function sisacuti($user,$hire){
    ...
    function add_process(){
        ...
        if ($cek == NULL){
            ...
            //get atasan pengaju        /.WHATEVER
            $ATS        = $this->pbl->get_employee($prs->IDEmployeeParent)->row();
            $ATSIN      = $ATS->InternalEmail == "" OR $ATS->InternalEmail == NULL ? "" : ",".$ATS->InternalEmail;
            $ATSEX      = $ATS->ExternalEmail == "" OR $ATS->ExternalEmail == NULL ? "" : ",".$ATS->ExternalEmail;
            
            $whcekposats['IDEmployee'] = $prs->IDEmployeeParent;
            $cekats  = $this->pbl->get_job_wharray($whcekposats)->row();
            if ($cekats->Position != "MANAGER"){
                // get manajernya pengaju   /.WHATEVER
                //$empl	= $this->pbl->get_job($userid)->row();
                $whmgr['Department']= $cekats->Department;
                $whmgr['Position']  = "MANAGER";
                $MGR	= $this->pbl->get_job_wharray($whmgr)->row();
                IF ($MGR != NULL){
                    $MGR	= $this->pbl->get_employee($MGR->IDEmployee)->row();
                    $MGRIN  = $MGR->InternalEmail == "" OR $MGR->InternalEmail == null ? "" : ",".$MGR->InternalEmail; 
                    $MGREX  = $MGR->ExternalEmail == "" OR $MGR->ExternalEmail == null ? "" : ",".$MGR->ExternalEmail; 
                }else{
                    $MGRIN  = "";
                    $MGREX  = "";
                }
            ...
            //
            $incc       = $this->inemail.$MGRIN.$ATSIN;
            $excc       = $this->exemail.$MGREX.$ATSEX;
