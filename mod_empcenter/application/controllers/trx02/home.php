...
class Home extends CI_Controller {
    ...
    function index() {
        $row = $this->pbl->get_datapersonal($this->User);
        ...
        if ($dept == '7' and $position == 'STAFF' and $flagleader == '0') {
            $flagsales = 'required';
        } else if($dept == '7' and $position == 'STAFF' and $flagleader == '1') {
            $flagsales = 'required';
        }else{
            $flagsales = 'norequired';
        }


        $data['flagsales'] = $flagsales;
...
//    checker ====================================================================== 
    function cek_job() {
        ...
        $idleader = $this->anti_xss($this->input->post("idleader"));
        ...
        $hasil[1] = $job->IDEmployeePTL == $idleader ? 0 : 1;
        $hasil[2] = $job->EmployeeStatus == $empstat ? 0 : 1;
        $hasil[3] = $job->Location == $jobloc ? 0 : 1;
        $hasil[4] = $job->JobGroup == $jobgrp ? 0 : 1;
        $hasil[5] = $job->Department == $depart ? 0 : 1;
        $hasil[6] = $job->Position == $jobpos ? 0 : 1;
        $hasil[7] = $job->Unit == $unitjob ? 0 : 1;
    ...
    function historydata($nip, $function, $historyto) {
        ...
        if ($historyto == 'personalemp') {
            ...
            $recordhispersonal = array(
                ...
                "IDEmployeePTL" => $rowpersonal->IDEmployeePTL,
            ...
            $recordhispersonald = array(
                ...
                "IDEmployeePTL" => $rowpersonal_d->IDEmployeePTL,
            ...
            $recordhispersonalpbl = array(
                ...
                "IDEmployeePTL" => $rowpersonalpbl->IDEmployeePTL,
                ...
                "F1s1" => $rowpersonalpbl->F1s1,
                "F1s2" => $rowpersonalpbl->F1s2,
                "F1s3" => $rowpersonalpbl->F1s3,
                "F1s4" => $rowpersonalpbl->F1s4,
                "F1s5" => $rowpersonalpbl->F1s5,
                "F2" => $rowpersonalpbl->F2,
                "F2f1" => $rowpersonalpbl->F2f1,
                "F2f2" => $rowpersonalpbl->F2f2,
            ...
            $recordhisjob = array(
                ...
                "IDEmployeePTL" => $rowjob->IDEmployeePTL,
