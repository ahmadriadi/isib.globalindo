...
class Menu_Model extends CI_Model {
    ...
    function get_menu_main($id=NULL,$p=NULL,$loc=NULL){
        if ($p == NULL){
            if ($id == NULL){
                $this->_db->where("ForApplication","dashboard");
            ...
            $query = $this->_db->query("SELECT * FROM $this->_tbl1 WHERE ForApplication='dashboard' AND HasSubMenu = 1");
...
//            $query = $this->_db->query("SELECT MenuDesc AS label, IDMenu AS idmenu FROM $this->_tbl1 WHERE HasSubMenu = 1");
    ...
    function get_child($idparent,$hassub=NULL){
      ...
      function get_child_main($idparent,$hassub=NULL){
        $this->_db->where("IDParent",$idparent);
        if($hassub == '0'){
            $this->_db->where("HasSubMenu", "0");
        }
